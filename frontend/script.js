...
// Determine API base URL dynamically based on where the app is running
const API_BASE = (window.location.port === '8080' || window.location.port === '5500' || window.location.protocol === 'file:')
    ? 'http://localhost:8001'
...
function getToken() {
    return localStorage.getItem('tubeRepliesToken');
}
...
function handleLogout() {
    localStorage.removeItem('tubeRepliesToken');
...
async function initVideosPage() {
    ...
    try {
        ...
        console.error(e);
        showToast('Error loading videos. Using demo data.', 'error');
    }
...
function buildVideoCard(v, index = 0) {
    ...
    return `
    <div class="video-card slide-up-item" style="animation-delay: ${index * 0.05}s" data-id="${v.id}" data-type="${v.type}">
...
async function initCommentsPage() {
    ...
    if (mini) mini.style.backgroundImage = `url(${v.thumbnail || 'https://img.youtube.com/vi/'+v.yt+'/mqdefault.jpg'})`;
    ...
    try {
        const res = await fetch(`${API_BASE}/comments?video_url=https://www.youtube.com/watch?v=${v.yt}`, { headers: authHeaders() });
        if (!res.ok) {
            if (res.status === 401) { handleLogout(); return; }
        ...
        // Build a lookup of AI replies from worth_replying (which has suggested_reply populated)
        const aiReplyMap = {};
        (data.worth_replying || []).forEach(c => {
            if (c.suggested_reply) aiReplyMap[c.comment_id] = c.suggested_reply;
        });

        COMMENTS = (data.all_comments || []).map(c => ({
            id: c.comment_id,
            author: c.author,
            time: c.published_at ? c.published_at.split('T')[0] : 'Just now',
            category: c.category || 'other',
            text: c.text,
            likes: c.likes || 0,
            replies: c.reply_count || 0,
            aiReply: aiReplyMap[c.comment_id] || c.suggested_reply || ''
        }));
    } catch (e) {
        console.error(e);
        showToast('Error loading comments. Using demo data.', 'error');
    }
...
function renderComments() {
    ...
    list.innerHTML = rows.map((c, i) => buildCommentCard(c, i)).join('');
...
function buildCommentCard(c, index = 0) {
    ...
    return `
    <div class="comment-card slide-up-item" style="animation-delay: ${index * 0.05}s" id="card-${c.id}" data-cat="${c.category}">
...
async function genReply(id) {
    ...
    let replyText = "";
    try {
        const res = await fetch(`${API_BASE}/generate_reply`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
    ...
    if (!replyText) {
        replyText = comment.aiReply || "Thanks for the comment! ❤️";
    }

    comment.aiReply = replyText;
...
async function postReply(id) {
    ...
    try {
        const res = await fetch(`${API_BASE}/reply`, {
            method: 'POST',
            headers: authHeaders(),
            body: JSON.stringify({ comment_id: id, reply_text: text })
        });
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (token) {
        localStorage.setItem('tubeRepliesToken', token);
        window.history.replaceState({}, document.title, window.location.pathname);
        showPage('videos');
    } else if (localStorage.getItem('tubeRepliesToken')) {
        showPage('videos');
    } else {
        showPage('landing');
    }
